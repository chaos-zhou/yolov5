        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    n = (det[:, 5] == c).sum()  # detections per class 类别
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string 

                # Write results 数据结果的，可以调整为输出json或者其他格式。
                for *xyxy, conf, cls in reversed(det):
                    width=float(xyxy[2])-float(xyxy[0])
                    height=float(xyxy[3])-float(xyxy[1])
                    st=width*height
                    rate=st/(im0.shape[0]*im0.shape[1])
                    if rate< 1/10:
                        continue
                    if not save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh,标准化坐标？目的？
...
def parse_opt():
    ...
    # parser.add_argument('--source', type=str, default='D:/Download/QQ_Images/', help='file/dir/URL/glob/screen/0(webcam)')
